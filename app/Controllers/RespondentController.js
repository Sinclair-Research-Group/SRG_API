...
class RespondentController {
    async addRespondent(respondentDetails, connection) {
        try {
            ...
            let query = `INSERT IGNORE INTO Respondent (lea, credential_program, first_name, last_name, program_role, years)
                         VALUES (?, ?, ?, ?, ?, ?)`;
            await connection.query(query, [Respondent.lea, Respondent.credential_program, Respondent.first_name, Respondent.last_name, Respondent.program_role, Respondent.years]);
